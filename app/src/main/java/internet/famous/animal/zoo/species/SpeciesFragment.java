...
public final class SpeciesFragment extends Fragment {
  private static final DataSubscriptionList subscriptions = new DataSubscriptionList();

  private SpeciesAdapter adapter;
  private Query<Species> speciesQuery;

  @Override
  public void onCreate(@Nullable Bundle savedInstanceState) {
    super.onCreate(savedInstanceState);
    adapter = new SpeciesAdapter(getContext());
    speciesQuery = BoxBuilder.build(getActivity(), Species.class).query().build();
  }
  ...
  public View onCreateView(
    ...
    ListView speciesListView = (ListView) view;
    speciesQuery
        .subscribe(subscriptions)
        .on(AndroidScheduler.mainThread())
        .observer(adapter::setSpeciesList);
